...
<title>delete_current_follow_stream</title>
...
<hr />
<?php
function draw_container($bid, $src, $dst, $fsname, $fsdescription, $update_date) {
  ...
  echo $fsname.'<br>';
  echo $fsdescription.'<br>';
  echo $update_date.'<br></td>';
  echo '<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
}
function show_follow_stream($fsid) {
  $find_pic = "select * from Follow_Stream F, FS_Board FS where F.fsid = FS.fsid and F.fsid = '$fsid'";
  $fp_result = mysql_query($find_pic);
  $fp_row = mysql_fetch_array($fp_result);
  $bid = $fp_row['bid'];
  $fsname = $fp_row['fsname'];
  $fsdescription = $fp_row['fsdescription'];
  $update_date = $fp_row['fsDate'];
  ...
  draw_container($bid, $src, $dst, $fsname, $fsdescription, $update_date);
  echo '<td><strong>Are you sure you want to delete current follow stream?</strong><br>';
  echo '<strong>Input password to process.</strong><br>';
  echo '<form name="form1" method="post" action="delete_current_follow_stream_process.php" id="form1">';
  echo '<input name="fsid" type="hidden" id="fsid" value="'.$fsid.'">';
  echo '<input name="password" type="password" id="password">';
  echo '<input name="submit" type="Submit" value="Okay"></td></p>';
}
$fsid = $_GET['fsid'];
show_follow_stream($fsid);
