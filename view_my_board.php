...
function read_follow_stream($username) {
  $find_board = 'select F.fsname as fsname, F.fsdescription as fsdescription, F.fsDate as fsDate, B.bid as bid from Account as A, User as U, Follow_Stream as F, FS_Board as FS, Board as B where A.uid = U.uid and U.uid = FS.uid and FS.bid = B.bid and FS.fsid = F.fsid and A.username = "'.$username.'"';
  #$find_board = mysql_escape_string($find_board);
  ...
  while ($row = mysql_fetch_array($fb_result)) {
	...
	draw_container($bid, $src, $dst, $follow_stream_name, $follow_stream_description, $follow_stream_date);
	if ($count%6 == 5) {
	  echo "</tr>";
	}
	$count ++;
